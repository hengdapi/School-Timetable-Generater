...
class BorderColorObject(QObject):
    """Border color object for border animation"""

    def __init__(self, parent):
        super().__init__(parent)
        self._borderColor = (
            parent._normalBorderColor()
            if hasattr(parent, "_normalBorderColor")
            else QColor(0, 0, 0, 0)
        )
...
class FluentAnimationProperObject(QObject):
    ...
    @classmethod
    def create(
        cls, propertyType: FluentAnimationProperty, parent=None
...
class ScaleSlideAnimation(QObject):
    ...
    def __init__(self, parent=None, orient=Qt.Orientation.Horizontal):
        ...
        self.orient = orient
        self._geometry = (
            QRectF(0, 0, 16, 3) if self.isHorizontal() else QRectF(0, 0, 3, 16)
        )
    ...
    def _startSlideAnimation(self, startRect, endRect, from_, to, dimension):
        ...
        self.slidePosAni1.setEasingCurve(
            FluentAnimation.createBezierCurve(0.9, 0.1, 1, 0.2)
        )
        self.slidePosAni2.setEasingCurve(
            FluentAnimation.createBezierCurve(0.1, 0.9, 0.2, 1.0)
        )

        self.slideLengthAni1.setDuration(200)
        self.slideLengthAni2.setDuration(400)
        self.slideLengthAni1.setEasingCurve(
            FluentAnimation.createBezierCurve(0.9, 0.1, 1, 0.2)
        )
        self.slideLengthAni2.setEasingCurve(
            FluentAnimation.createBezierCurve(0.1, 0.9, 0.2, 1.0)
        )
    ...
    def _startCrossFadeAnimation(self, startRect, endRect):
        ...
        isNextBelow = (
            ...
            else endRect.x() > startRect.x()
        )

        if self.isHorizontal():
            dim = endRect.width()
            startGeo = QRectF(
                endRect.x() + (0 if isNextBelow else dim),
                endRect.y(),
                0,
                endRect.height(),
            )
            ...
            startGeo = QRectF(
                endRect.x(),
                endRect.y() + (0 if isNextBelow else dim),
                endRect.width(),
                0,
            )
