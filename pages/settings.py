...
class Settings(QFrame):
    ...
    def pick_lessons_info(self):
        ...
        if user_info_file:
            try:
                ...
                self.cfg.lessons_info.value=user_info.to_dict(orient="records")
                for class_ in self.cfg.lessons_info.value:
                ...
                self.cfg.teachers_info.value=list(new_teachers)
                save_settings(self)
            except Exception as e:
                save_settings(self,False,str(e))
        else:
            if self.cfg.lessons_info.value!="":
                self.cfg.lessons_info.value=pd.DataFrame(self.cfg.lessons_info.value).to_json(orient="records", lines=False, force_ascii=False)
        ...
        for string in name:
            if string[0]=="{" and string[-1]=="}":
                ...
                if "subject" in string_name:
                    items=self.cfg.subjects_info.value
                elif "time" in string_name:
                    items=["上午","下午"]+[days[day]+["上午","下午"][time] for day in range(1,6) for time in range(2)]+[days[day] for day in range(1,6)]+self.times
                elif "number" in string_name:
                    items=[str(i) for i in range(1,len(self.cfg.lessons_info.value)+1)]
                elif "teacher" in string_name:
                    items=self.cfg.teachers_info.value
                elif "class" in string_name:
                    items=["单数班级","双数班级"]+[clas["班级"] for clas in self.cfg.lessons_info.value]
                else:
                    items=["无可选项"]
    ...
    def add_rule(self):
        ...
        if success:
            self.cfg.rules.value.append(new_rule)
    ...
    def remove_rule(self):
        try:
            ...
            self.cfg.rules.value.remove(selected_rule.data(Qt.UserRole))
            self.rule_list.takeItem(self.rule_list.row(selected_rule))
            self.remove_rule_button.setEnabled(len(self.cfg.rules.value))
            save_settings(self)
        except Exception as e:
            save_settings(self,False,str(e))
    ...
    def __init__(self,parent=None):
        ...
        afternoon_class_num=RangeSettingCard(self.cfg.afternoon_class_num,FluentIcon.FLAG,title="每天下午上课数量",content="学校每天下午的上课数量")
        ...
        lessons_info=pd.DataFrame(self.cfg.lessons_info.value)
        ...
        rule_types=self.cfg.rule_types.value
        self.times=[day+generate_time(lesson) for day in days[1:] for lesson in range(1,self.cfg.morning_class_num.value+self.cfg.afternoon_class_num.value+1)]
        ...
        for i in range(len(self.cfg.rules.value)):
            rule=self.cfg.rules.value[i]
